...
import { getSystemEdgeBadge } from '../get-system-edge-badge';
...
export function LabelEdge({
  ...
  const systemBadge = getSystemEdgeBadge(label);
  const isInteractive = !systemBadge;
  const content = systemBadge ? null : icon ? <Icon name={icon} /> : label;
  ...
  const labelProps = {
    ...
    selected,
    icon,
    isStaticBadge: !!systemBadge,
    badgeIcon: systemBadge,
    onMouseEnter,
    onMouseLeave,
  };
  ...
  return (
    <>
      <EnhancedBaseEdge id={id} path={edgePath} style={style} isInteractive={isInteractive} />
